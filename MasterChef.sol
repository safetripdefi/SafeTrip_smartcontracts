...
abstract contract SafetripToken is Context, IBEP20, Ownable {
    ...
    function changedecayBurnrate(uint256 _newdecayBurnrate) public onlyOwner returns (bool) {
        ...
        require(_newdecayBurnrate >= mindecayBurnrate, "too low value");
...
contract MasterChef is Ownable {
    ...
    uint256 public constant mintokenperblock = 0; // 0 token per block
    ...
    uint256 public constant maxwithdrawfee = 500; // 5% - 500/10000
    ...
    event SetDepositFeeAddress(address indexed user, address indexed newAddress);
    ...
    event GlobalDecayToken(address indexed user);
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint256 _depositFeeBP, uint256 _withdrawFeeBP, bool _withUpdate) public onlyOwner {
        ...
        require(_depositFeeBP >= mindepositfee, "add: invalid deposit fee basis points");
        require(_withdrawFeeBP <= maxwithdrawfee, "add: invalid withdraw fee basis points");
        require(_withdrawFeeBP >= minwithdrawfee, "add: invalid withdraw fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        emit Add(msg.sender, _allocPoint, _lpToken, _depositFeeBP, _withdrawFeeBP);
    }

    // Update the given pool's TRIP allocation point and deposit fee. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint256 _depositFeeBP, uint256 _withdrawFeeBP, bool _withUpdate) public onlyOwner {
        require(_depositFeeBP <= maxdepositfee, "set: invalid deposit fee basis points");
        require(_depositFeeBP >= mindepositfee, "set: invalid deposit fee basis points");
        require(_withdrawFeeBP <= maxwithdrawfee, "set: invalid withdraw fee basis points");
        require(_withdrawFeeBP >= minwithdrawfee, "set: invalid withdraw fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        emit Set(msg.sender, _allocPoint, _depositFeeBP, _withdrawFeeBP);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        trip.mint(address(bus), tripReward);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            if(pool.withdrawFeeBP > 0){
                ...
                uint256 left = _amount.sub(withdrawFee);
                pool.lpToken.safeTransfer(withdrawfeeAddress, withdrawFee);                
    ...
    function safeTripTransfer(address _to, uint256 _amount) internal {
        bus.safeTripTransfer(_to, _amount);
    }
    ...
    function setDepositFeeAddress(address _newdepositfeeAddress) public {
        require(msg.sender == depositfeeAddress, "setFeeAddress: FORBIDDEN");
        depositfeeAddress = _newdepositfeeAddress;
        emit SetDepositFeeAddress(msg.sender, _newdepositfeeAddress);
    }

    function setWithdrawFeeAddress(address _newwithdrawfeeAddress) public {
        require(msg.sender == withdrawfeeAddress, "setFeeAddress: FORBIDDEN");
        withdrawfeeAddress = _newwithdrawfeeAddress;
        emit SetWithdrawFeeAddress(msg.sender, _newwithdrawfeeAddress);
    }

    //
    function changetokensPerBlock(uint256 _tripPerBlock) public onlyOwner {
        require(_tripPerBlock <= maxtokenperblock, "too high value");
        require(_tripPerBlock >= mintokenperblock, "too low value");
        massUpdatePools();
        tripPerBlock = _tripPerBlock;
    ...
    //globaldecay tokens
    function globaldecaytoken() public onlyOwner returns (bool) {
        trip.globalDecay();
        emit GlobalDecayToken(msg.sender);
        return true;
    }
}